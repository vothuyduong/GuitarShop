        ...
        <!-- /. ROW  -->
         <%
	        if(request.getParameter("err") != null) {
	    		...
	    		if(err.equals("1")) {
	    			out.print("<span style=\"color: red; margin-left: 20px;\">Tên cần tìm rỗng!</span>");
	    		}
	    	}
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6" style="text-align: right;">
                                    <form method="post" action="<%=request.getContextPath()%>/admin/order/search">
                                        ...
                                        <input type="search" name="key" value="<%if(request.getAttribute("key") != null) out.print(request.getAttribute("key")); %>" class="form-control input-sm" placeholder="Nhập tên khách hàng" style="float:right; width: 300px;" />
                            ...
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                %>
                                    <tr>
                                        ...
                                        <td class="center"><%=item.getNgayTao() %></td>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6" style="text-align: right;">
                                    <div class="dataTables_paginate paging_simple_numbers" id="dataTables-example_paginate">
                                        ...
                                        if(trangHT > 1) {
                                        %>
                                            <li class="paginate_button previous" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_previous"><a href="<%=request.getContextPath()%>/admin/orders?page=<%=trangHT-1%>&key=<%=key%>">Trang trước</a></li>
                                            <%
                                        }
                                            	for(int i = 1; i <= soTrang; i++) {
                                            		if(i == trangHT) {
                                            %>
                                            <li class="paginate_button active" aria-controls="dataTables-example" tabindex="0"><a href="<%=request.getContextPath()%>/admin/orders?page=<%=i%>&key=<%=key%>"><%=i %></a></li>
                                            <%} else { %>
											<li class="paginate_button" aria-controls="dataTables-example" tabindex="0"><a href="<%=request.getContextPath()%>/admin/orders?page=<%=i%>&key=<%=key%>"><%=i %></a></li>
											<%
                                            	}}
                                            	if(trangHT < soTrang) {
											%>
                                            <li class="paginate_button next" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_next"><a href="<%=request.getContextPath()%>/admin/orders?page=<%=trangHT+1%>&key=<%=key%>">Trang tiếp</a></li>
                                            <%} %>
                                        </ul>
                                    </div>
                                </div>
